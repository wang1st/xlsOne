# AMD64 Linux DEB 打包指引

**目标：** 在 x86_64 Linux 机器上构建 `xlsone-1.0.2-linux-amd64.deb`，确保所有必要函数库都打包进去，安装后即可运行。
...
```bash
# 系统要求：x86_64 的 Ubuntu 20.04/22.04、Debian、UOS 或 Kylin
...
wget "https://github.com/deepin-community/sig-deepin-shared-libs/releases/download/Qt5.15.10-OpenGLES%3D%3D5.15.10%2Bszbt2/qt5.15-gles_231205_amd64.tar.xz"

tar -xf qt5.15-gles_231205_amd64.tar.xz
```
...
## 5. 下载对应 ICU 库（以 ICU 63 amd64 为例）
...
```text
~/xlsone/cpp/deepin-qt5.15/icu63/usr/lib/x86_64-linux-gnu/
├── libicudata.so.63
├── libicui18n.so.63
└── libicuuc.so.63
```
...
dpkg-deb -x ~/xlsone/cpp/build-linux-release/xlsone-1.0.2-linux-amd64.deb deb_check
...
```bash
# 找一台没有 Qt5 开发环境的 x86_64 机器
sudo dpkg -i xlsone-1.0.2-linux-amd64.deb
xlsone
```
