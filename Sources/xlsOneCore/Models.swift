...
public struct MergedCell: Equatable, Sendable {
    ...
    public static func from(
        ...
        if validCells.isEmpty {
            ...
            return MergedCell(
                ...
                decision: defaultDecision(
                    for: type,
                    reasons: [LocaleManager.loc("所有来源单元格均为空或缺失，按空标签保留原始位置")],
                    confidence: 1.0,
                    isSuspicious: false
                )
            )
        ...
        if row == 0 {
            ...
            return MergedCell(
                ...
                decision: defaultDecision(
                    for: type,
                    reasons: [LocaleManager.loc("首行按表头处理，强制视为标签列")],
                    confidence: 1.0,
        ...
        if allNumericValuesAreZero,
           ...
           !labelSemantic {
            ...
            return MergedCell(
                ...
                decision: defaultDecision(
                    for: type,
                    reasons: [LocaleManager.loc("所有非空来源均为 0，按可累加单元格求和处理")],
                    confidence: 0.9,
                    isSuspicious: false
                )
            )
        ...
        if allNumericValuesAreIdenticalNonZeroIntegers,
           ...
           !metricSemantic {
            ...
            return MergedCell(
                ...
                decision: defaultDecision(
                    for: type,
                    reasons: [LocaleManager.loc("所有来源为相同非零整数，且无明确可累加语义，按标签处理")],
                    confidence: 0.9,
                    isSuspicious: false
                )
            )
        ...
        if blanksWithNumericValues {
            decisionReasons.append(LocaleManager.loc("部分来源为空或缺失，非空来源均为数值，空值按 0 参与可累加判断"))
        }
        ...
        if let left = leftCell {
            ...
            if isSummary && selfFP.isNumeric {
                ...
                return MergedCell(
                    ...
                    decision: defaultDecision(
                        for: type,
                        reasons: decisionReasons + [LocaleManager.loc("左邻列命中合计语义，强制按求和处理")],
                        confidence: 0.98,
                        isSuspicious: false
                    )
                )
        ...
        if amountScore > 0.5 {
            ...
            if accumulable {
                ...
                return MergedCell(
                    ...
                    decision: defaultDecision(
                        for: type,
                        reasons: decisionReasons + [LocaleManager.loc("综合得分偏向数值，且通过可累加性检查")],
                ...
                return MergedCell(
                    ...
                    decision: defaultDecision(
                        for: type,
                        reasons: decisionReasons + [LocaleManager.loc("数值得分较高，但可累加性检查失败，回退为标签")],
            ...
            return MergedCell(
                ...
                decision: defaultDecision(
                    for: type,
                    reasons: decisionReasons + [LocaleManager.loc("综合得分偏向标签")],
                    confidence: baseConfidence,
            ...
            if selfFP.isNumeric {
                ...
                if accumulable {
                    ...
                    return MergedCell(
                        ...
                        decision: defaultDecision(
                            for: type,
                            reasons: decisionReasons + [LocaleManager.loc("边界场景下仍满足可累加性，按求和处理")],
            ...
            return MergedCell(
                ...
                decision: defaultDecision(
                    for: type,
                    reasons: decisionReasons + [LocaleManager.loc("分数接近，按保守策略作为标签处理")],
    ...
    private static func determineFirstColumn(
        ...
        if allNumericValuesAreZero {
            ...
            return MergedCell(
                ...
                decision: defaultDecision(
                    for: type,
                    reasons: [LocaleManager.loc("首列所有非空来源均为 0，按可累加单元格求和处理")],
                    confidence: 0.9,
                    isSuspicious: false
                )
            )
        ...
        if allNumericValuesAreIdenticalNonZeroIntegers, blankSourceCount == 0 {
            ...
            return MergedCell(
                ...
                decision: defaultDecision(
                    for: type,
                    reasons: [LocaleManager.loc("首列所有来源为相同非零整数，按标签处理")],
                    confidence: 0.9,
                    isSuspicious: false
                )
            )
        ...
        if let dominant = profile.dominantFingerprint {
            ...
            case .integerCode:
                ...
                return MergedCell(
                    ...
                    decision: defaultDecision(
                        for: type,
                        reasons: [LocaleManager.loc("首列主导格式为整数编码，按标签处理")],
                        confidence: 0.95,
                        isSuspicious: Set(validCells.map(\.1.value)).count > 1
                    )
                )
            case .strongNumeric, .integerWide:
                if profile.dominantRatio >= 0.8 {
                    ...
                    return MergedCell(
                        ...
                        decision: defaultDecision(
                            for: type,
                            reasons: [LocaleManager.loc("首列以数值格式为主且一致性高，按求和处理")],
                            confidence: 0.84,
                            isSuspicious: false
                        )
                    )
        ...
        return MergedCell(
            ...
            decision: defaultDecision(
                for: type,
                reasons: [LocaleManager.loc("首列采用保守策略，按标签处理")],
                confidence: 0.7,
                isSuspicious: Set(validCells.map(\.1.value)).count > 1
            )
        )
    ...
    private static func determineSingleValue(
        ...
        if let numericValue,
           ...
           (formatSuggestsNumeric || sameColumnSuggestsNumeric || leftSuggestsNumeric || metricSemantic || zeroWithBlankBias) {
            ...
            var reasons = [LocaleManager.loc("仅有一个非空数值，未直接按单值处理")]
            if formatSuggestsNumeric {
                reasons.append(LocaleManager.loc("单元格格式倾向数值"))
            }
            if sameColumnSuggestsNumeric || weakSameColumnNumeric {
                reasons.append(LocaleManager.loc("同列上下文倾向数值"))
            }
            if leftSuggestsNumeric {
                reasons.append(LocaleManager.loc("左邻语义倾向可累加"))
            }
            if metricSemantic {
                reasons.append(LocaleManager.loc("计量语义与同列上下文共同支持求和"))
            }
            if zeroWithBlankBias {
                reasons.append(LocaleManager.loc("零值与空值/缺失并存，按可求和单元格处理"))
            }
        ...
        let type = CellType.single(cell.value)
            var reasons = [LocaleManager.loc("仅有一个非空来源值，按单值显示")]
        if isCodeLike || leftSuggestsLabel || neighborContext.labelTendency > neighborContext.numericTendency {
            reasons.append(LocaleManager.loc("格式或上下文更偏向标签/编码"))
        } else if cell.numericValue != nil {
            reasons.append(LocaleManager.loc("暂无足够格式或同列证据支持求和"))
        }
    ...
    private static func defaultDecision(
        ...
        MergedCellDecision(
            ...
            confidence: confidence,
            decisionReasons: reasons ?? [LocaleManager.loc("未提供额外判定解释")],
            isSuspicious: isSuspicious
        )
    }
...
extension FormatFingerprint {
    var descriptionText: String {
        switch self {
        case .strongNumeric:
            return LocaleManager.loc("强数值")
        case .integerWide:
            return LocaleManager.loc("宽整数")
        case .integerCode:
            return LocaleManager.loc("整数编码")
        case .chineseText:
            return LocaleManager.loc("中文文本")
        case .alphaText:
            return LocaleManager.loc("英文文本")
        case .date:
            return LocaleManager.loc("日期")
        case .dashMarker:
            return LocaleManager.loc("占位符")
        case .empty:
            return LocaleManager.loc("空值")
        case .mixed:
            return LocaleManager.loc("混合格式")
        }
    }
}
