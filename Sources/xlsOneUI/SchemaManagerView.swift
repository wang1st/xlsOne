...
struct SchemaManagerView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack {
                ...
                Button("清除当前调整记忆", role: .destructive) {
    ...
    private var emptyState: some View {
        VStack(spacing: 16) {
            ...
            Text("在汇总结果中修正单元格后，系统会为当前同构结构记住这些调整；也可以导入同构结构的调整记忆。")
                .font(.caption)
                .foregroundStyle(.secondary)
...
private struct AdjustmentMemoryDetailView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                detailRow("名称", schema.name)
                detailRow("更新", Self.dateFormatter.string(from: schema.updatedAt))
                detailRow("工作表", sheetSummary)
                detailRow("修正", "\(schema.cellOverrides.count) 处")
    ...
    private var sheetSummary: String {
        ...
        return schema.fingerprint.sheetName.isEmpty ? "当前同构结构" : schema.fingerprint.sheetName
    }
