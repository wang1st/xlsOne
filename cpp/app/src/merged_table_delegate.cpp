...
void MergedTableDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    ...
    const bool suspicious = index.data(MergedTableModel::SuspiciousRole).toBool();

    QColor background = Qt::white;
    if (kind == xlsone::CellKind::Sum) {
        background = t.sumSoft;
    } else if (kind == xlsone::CellKind::Mixed) {
    ...
    if (overridden || suspicious) {
        const QColor marker = overridden ? t.adjusted : QColor(207, 142, 39);
